    ...
    if request.method == 'POST':
        ...
        if cursor.fetchone():
            ...
            return render_template('admin_edit_user.html',
                                   user=(name, surname, phone, email, username, password, newsletter),
...
def admin_marketing():
    ...
    if request.method == 'POST':
        ...
        <body style="text-align:center; padding:40px;">
            ...
            <a href="{url_for('admin_dashboard')}"><button>← Torna alla Dashboard</button></a>
        </body>
        </html>
        """

    # Mostra max 100 utenti opt-in
    cursor.execute("SELECT name || ' ' || surname, email FROM users WHERE newsletter_optin = 1 ORDER BY id DESC LIMIT 100")
    users = cursor.fetchall()
    conn.close()
...
""", (user_id, service, date, time, assigned_barber))
        ...
        # 🔁 Aggiorna il gender a 'uomo' se non è ancora impostato
        cursor.execute("""
    UPDATE users SET gender = 'uomo' 
    WHERE id = ? AND (gender IS NULL OR gender = '')
""", (user_id,))
...
def book_hair():
    ...
    if request.method == 'POST':
        ...
        # 🔁 Aggiorna il gender a 'donna' se non è ancora impostato
        cursor.execute("""
    UPDATE users SET gender = 'donna' 
    WHERE id = ? AND (gender IS NULL OR gender = '')
""",    (user_id,))
